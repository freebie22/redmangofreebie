...
const PaymentForm = ({ data, userInput }: orderSummaryProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.error) {
      ...
      toastNotify("Your order has been created. Wait for details!", "success")
      let grandTotal = 0;
      let totalItems = 0;
  ...
  return (
    <form onSubmit={handleSubmit}>
      <PaymentElement />
      <button disabled={isProcessing} className="btn btn-success mt-5 w-100">{isProcessing ? (<div><MiniLoader></MiniLoader></div>) : "Submit"}</button>
    </form>
  );
};
